...
from fairseq import options, progress_bar, tasks, utils, pybleu, checkpoint_utils
...
def eval_bleu_score(
    ...
    print(task.target_dictionary.pad())
    print('# Initialize generator')
    ##beam_size=beam,---> 
    scorer = pybleu.PyBleuScorer()
    translator = SequenceGenerator(
        [model], tgt_dict=task.target_dictionary, min_len=min_len, 
        stop_early=stop_early, normalize_scores=normalize_scores,
        len_penalty=lenpen,
        sampling=False,
    )

    print('# translate_corpus')
    print([eval_data.src.get_original_text(
    ...
    return scorer.score(ref, out) #sacrebleu.corpus_bleu(out, [ref], force=True, tokenize="none")
...
def main(args):
    ...
    ##task.load_dataset('valid') train-subset
    ...
    #model, _ = checkpoint_utils.load_model_ensemble(args.restore_file.split(':'), task)
    models, _ = utils.load_ensemble_for_inference(args.restore_file.split(':'), task)
    model = models[0].cuda()
    ...
    #    trainer.train_step(dummy_batch=True)
    ...
    for i in range(0, 10):
        ...
        print('| done estimating head importance in {:.1f} seconds'.format(
            prune_meter.sum))
        print('save in ',args.restore_file)
        torch.save(head_stats, f"{os.path.dirname(args.restore_file)}/heads_stats.bin")
        #extra_state = {
        #'train_iterator': epoch_itr.state_dict(),
        #'val_loss': 100,}
        #print('saving model in {}/pruning_{}.pt'.format(os.path.dirname(args.restore_file),i))
        #trainer.save_checkpoint('{}/pruning_{}.pt'.format(os.path.dirname(args.restore_file),i),extra_state)
        # Print
...
#    for i in range(0, 10):
        ...
        mask_heads(model, to_prune, args.transformer_mask_rescale)
        print('BLEU не работает')
        #bleu = eval_bleu_score(
        #    model,
        #    task,
        #    eval_data = task.dataset(args.valid_subset),
        #    beam=args.beam,
        #    replace_unk=args.replace_unk,
        #    lenpen=args.lenpen,
        #    buffer_size=100,
...
def estimate_head_importance(args, trainer, task, epoch_itr):
    ...
    if args.n_pruning_steps > 0:
        itr = list(islice(itr, args.n_pruning_steps))
